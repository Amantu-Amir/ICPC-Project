...
char s[210][210];
int row, col;
int bfs(int x, int y, vector<pii>&v, int flag){
    int vis[205][205];
    ...
    q.push({x, y});
    while(!q.empty()){
        pii par = q.front(); q.pop();
        for(int i=0; i<4; i++){
            int tx = par.ff + fx[i];
            int ty = par.ss + fy[i];
            if(flag == 1 && s[tx][ty] == '*') continue;
            if(tx >= 0 && tx < row && ty >= 0 && ty < col && s[tx][ty] != '#' && !vis[tx][ty]){
                vis[tx][ty] = vis[par.ff][par.ss] + 1;
                if(flag == 1 && s[tx][ty] == 'D'){
                    res = min(res, vis[tx][ty]);
                }
                if(s[tx][ty] == '*' && flag == 0){
                    if(res >= vis[tx][ty]){
                        v.pb({tx, ty});
...
int32_t main(){
    ...
    //fast();
    int tc=1; scanf("%lld", &tc);
    for(int T=1; T<=tc; T++){
        scanf("%lld %lld", &row, &col);
        int dx, dy, px, py, cnt=0;
        for(int i=0; i<row; i++)
            scanf("%s", s[i]);
        for(int i=0; i<row; i++){
            for(int j=0; j<col; j++){
        ...
        vector<pii>vp,vd,just;
        int P = bfs(px, py, vp, 0);     //search teleporting cell from P
        int D = bfs(dx, dy, vd, 0);     //search teleporting cell from D
        int aa = bfs(px, py, just, 1);   //search D from P without visit teleporting cell
        CAS(T); sp;
        if(cnt == 1){
            if(aa >= (1<<28)){
                printf("impossible\n");
            ...
            int add = 1;
            if(vp.size() == 1 && vd.size() == 1){
                if(vp[0] == vd[0]) add++;
            }
            int ans = min(P+D+add, aa);
